...
def check_line_for_errors(
    ...
    """检查单行字符串中FTB颜色码 '&' 后是否包含非法字符或行尾 '&'"""
    # FTB颜色码 '&' 后跟 'a' 到 'v' 或 '0' 到 '9' 或 '\' 或 '#' 是合法的
    # 非法字符是除了这些合法字符和空白符以外的任何字符
    ...
    if line != stripped_line and not line.strip().endswith("\\&"):
         yield ErrorRecord(file_path, key, line.strip(), "行尾包含未转义的 '&'")
    ...
    try:
        ...
        def process_value(value: Union[str, list, dict], parent_key: str = ""):
            ...
            if isinstance(value, str):
                # Split by newline to check each line separately
                for i, line in enumerate(value.split("\n")):
                    # Use line number or partial value for key context in multi-line strings
                    line_key = f"{parent_key}[line {i+1}]" if "\n" in value else parent_key
                    yield from check_line_for_errors(
                        line, file_path, line_key # Pass original line, not stripped here
                    )
...
def generate_html_report(
    ...
    print(f"正在生成 HTML 报告: {output_path}")
...
<body>
    ...
    for error in errors:
        ...
        if error.error_message == "行尾包含未转义的 '&'" and escaped_value.endswith('&'):
